...
final class Mapper {
    ...
    func mapBoxOfficeMovieData() async -> [BoxOfficeMovie] {
        boxOfficeData = await movieRepository.prepareBoxOfficeDataRequest()
    ...
    func mapBoxOfficeDetailData(movie: String) async -> MovieDetailInfo {
        detailBoxOfficeData = await movieRepository.prepareDetailMovieDataRequest(movieCode: movie)
